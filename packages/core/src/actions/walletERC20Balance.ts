...
export const walletERC20Balance: Action = {
  ...
  schema: z.object({
    ...
    token: z
      .string()
      .describe('The token address or symbol to check the balance of'),
  }),
  ...
  handler: async (agent, input) => {
    ...
    if (balances.result[0]) {
      return {
        status: 'success',
        balance: balances.result[0].totalBalance,
        message: `Wallet balance: ${balances.result[0].totalBalance} ${balances.result[0].symbol}`,
      }
    }
    return {
      status: 'error',
